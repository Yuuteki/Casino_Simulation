...
- Casino/system credit v0.1 is settled as a fictional in-game credit system: credit score ranges from 0 to 100, starts at 60, and the first daily check-in grants +1 up to 100. Let `D` be the pending daily base chip supply; maximum borrowing is 0 for credit 0-19, 3D for 20-39, 6D for 40-59, 10D for 60-79, and 15D for 80-100.
- Credit borrowing uses one total debt pool with daily compound interest based on total debt, a 7-day due date that additional borrowing does not refresh, a -20 credit-score penalty if unpaid at due date, and one monthly forgiveness that clears debt but restricts new borrowing for the rest of that month. The exact chip value of `D` remains pending.
...
- The frozen first playable MVP is a local blackjack vertical slice: local round state machine, local official-chip ledger/profile, greybox table and screen-space UI, basic dealer/AI play, and repeatable automated checks. Friends Host/Client networking, casino overview, seating cinematic, avatar drawing implementation, final art, progression, roulette, sic bo, poker, slots, and full credit-system UI are later milestones unless explicitly re-scoped.
